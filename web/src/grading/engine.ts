...
import { GRADE_THRESHOLDS } from './types.ts';
import type { DimensionScore, Grade, GradeResult, GradeScenario, VetoHit } from './types.ts';
import { THRESHOLDS } from './thresholds.ts';
import type { DimensionKey } from './thresholds.ts';
 ...
 * @param vetoes 触发的否决规则（引擎不重复计算，全部应用取最严格）
