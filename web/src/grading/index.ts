/**
 * 评级系统统一入口（Kuantix 实现，规格对齐 docs/06 §3.4）。
 *
 * 三个场景函数：
 *   gradePerformance(perf)   — 单标的回测（performance 记录，6 维度）
 *   gradePortfolio(equity)   — 组合回测（净值曲线重算，5 维度）
 *   gradeGridPoint(point)    — 参数寻优（4 字段子集，4 维度）
 *
 ...
 * - 样本不足（< 10 笔）不再否决到 D：净值类维度依然可信，仅降权 win_rate/profit_factor。
 */
export function applyVetoes(ctx: VetoContext): {
  ...
  if (enoughTrades && ctx.winRate !== undefined && ctx.winRate !== null && ctx.winRate < 0.25) {
    vetoes.push({
 ...
 * 是从净值曲线算出来的，依然高度可信。降权而非否决，是统计上更合理的处理。
 *
...
/** 单标的评级所需的 performance 字段（其余字段忽略）。 */
export interface SinglePerformanceInput {
  calmar?: number | null;
 ...
 * max_dd_duration 不参与加权：该字段口径与「套牢多久回本」的产品直觉不符，波动率已足够。
 */
export function gradePerformance(perf: SinglePerformanceInput): GradeResult {
  const num = (v: number | null | undefined): number => (typeof v === 'number' && Number.isFinite(v) ? v : 0);

  const dimensions: DimensionScore[] = [
    scoreDimension('calmar', num(perf.calmar), 0.18),
    scoreDimension('max_drawdown', num(perf.max_drawdown), 0.17),
    scoreDimension('win_rate', num(perf.win_rate), 0.17),
    scoreDimension('profit_factor', num(perf.profit_factor), 0.18),
    scoreDimension('sharpe', num(perf.sharpe), 0.15),
    scoreDimension('volatility', num(perf.volatility), 0.15),
  ];

  const { vetoes, insufficientSample, isLosing } = applyVetoes({
    profitFactor: perf.profit_factor,
    totalTrades: num(perf.total_trades),
 ...
 *
 * 否决规则中只有「回撤类」能生效（无交易笔数/利润因子）。
 *
 * @param equity 组合净值曲线（B4 combined / P3 combined_equity）
 */
export function gradePortfolio(equity: BacktestEquityPoint[]): GradeResult {
...
/** 寻优网格点结果（O3 results/best 元素，对齐 docs/06 §2.3）。 */
 ...
 */
export function gradeGridPoint(point: GridPointInput, totalTradesOverride?: number): GradeResult {
...
export type { Grade, GradeResult, DimensionScore, VetoHit, GradeMeta, GradeScenario } from './types.ts';
